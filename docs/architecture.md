...
Manual reminder ordering is stored in `REMCDManualSortHint_v1` rows exposed by ReminderKit as `REMManualOrdering`. For ordinary lists, `reminder-move` updates order through `REMListChangeItem.insertReminderChangeItem:beforeReminderChangeItem:` or `insertReminderChangeItem:afterReminderChangeItem:`. For custom smart lists, it rewrites the relevant ordered object IDs through `REMSmartListChangeItem.updateManualOrdering`. Smart-list moves are restricted to an existing manual-order record and, when sections are present, the reminder's current section. RemCTL reads the local database only to resolve and verify the resulting order; it never writes SQLite directly.
